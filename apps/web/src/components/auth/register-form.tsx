...
export function RegisterForm() {
  ...
  const m = useMutation({
    mutationFn: () => api.auth.register({ email, password, displayName: displayName || undefined }),
    onSuccess: (data) => {
      setSession(
        { ...data.user, isVerified: false, createdAt: new Date().toISOString(), lastLoginAt: null },
        data.tokens,
      );
      router.push('/map');
    },
  ...
  return (
    <div className="w-full max-w-sm">
      <h1 className="font-display text-3xl text-text">Create account</h1>
      <p className="mt-1 font-mono text-xs text-text-subtle">
        Free tier ships unlimited AOIs and 5 alert rules. Pro tiers unlock bulk export and the opportunity scoring engine.
      </p>
      <form className="mt-6 space-y-3" onSubmit={(e) => { e.preventDefault(); m.mutate(); }}>
