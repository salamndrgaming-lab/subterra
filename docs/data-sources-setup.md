...
| `SUBTERRA_WELLS_OK_URL` | Point | Oklahoma OCC. Committed default is the OCC RBDMS_WELLS hosted FeatureServer (gis.occ.ok.gov). Override if the layer index is wrong. |
| `SUBTERRA_WELLS_NM_URL` | Point | New Mexico OCD. Committed default is NMOCD_Wells_V3 (mapservice.nmstatelands.org). If layer 0 isn't the well points, set this to the correct `/MapServer/<n>/query`. |
| `SUBTERRA_PRODUCTION_URL` | Bulk **CSV / zip** file URL(s) | **Per-well monthly production** → features.db `production` table → the well-detail sparkline + Arps decline fit. States publish production as bulk files, not APIs. Set a **comma-separated list** of file URLs (plain `.csv` or a `.zip` wrapping one). Columns are auto-mapped (api / period / oil / gas / water / days; CO's `api_county_code`+`api_seq_num` are combined into an API). Capped at 8M rows. Inert until set. **Colorado ECMC** ships annual production CSV/zips (1999→) — grab a couple of recent years from ecmc.colorado.gov → Data → Downloads → "Oil and Gas Well Analytical Data / Production". **Oklahoma OCC** + **Texas RRC** have their own bulk files. `SUBTERRA_PRODUCTION_CO_URL` / `_ND_URL` are also honored. |
